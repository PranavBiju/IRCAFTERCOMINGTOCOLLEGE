...
rcl_node_t node;
rcl_subscription_t cmd_sub;            // <-- CHANGED
rclc_executor_t executor;
rcl_allocator_t allocator;
rclc_support_t support;

geometry_msgs__msg__Twist cmd_msg;     // <-- CHANGED
...
// ---------------- CMD_VEL CALLBACK (ONLY REAL CHANGE) ----------------
...
void cmdVelCallback(const void * msg) {
  ...
  // DEBUG: same velocity to all wheels
  target_v1 = vx;
  target_v2 = vx;
  target_v3 = vx;
  target_v4 = vx;
  ...
  Serial.print("cmd_vel received | linear.x = ");
  Serial.println(vx);
}
...
void setup() {
  ...
  rclc_node_init_default(&node, "pid_drive_cmdvel_debug", "", &support);
